...
So, for example, the [crunch](https://github.com/Crunch-io/rcrunch) package, the subject of our discussion at work, formally lists these five dependencies
...
<!-- Following on the `purrr` example from above: is it bad design, or at a minimum just really strange, that the signature for `mapply` puts the function first and mixes title-case, all-caps, and dot-separated argument names:

```
mapply <- function (FUN, ..., MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE)
```

while `sapply`, like the other `*apply`s, puts the data first and function second, but has "simplify" lower-cased as an argument?

```
sapply <- function (X, FUN, ..., simplify = TRUE, USE.NAMES = TRUE)
```

Absolutely. Should you instead use `purrr` in your analysis work so that you have a more standardized interface to this kind of functional programming? Feel free. But if you're moving data around inside a function in your package, and an `lapply` or `vapply` would do the trick, you should think twice about reaching for an external library and requiring all of your package users to install it and its dependencies. The standard is different when you're asking other people to run your code on their machines. -->
