...
You could throw `template_articles()`, the third template function, in there for good measure, if you've got multiple vignettes in your package and want to customize the display of `articles/index.html`. I personally didn't bother because by default, there is no link to that page anywhere that I can find. But there's no harm in including it in your YAML.

Now that you have a basic YAML file, you can start customizing it. The docs for [`build_site()`](http://hadley.github.io/pkgdown/reference/build_site.html) give an overview of what is possible, and you can get pretty far with trial-and-error and by looking at other `pkgdown` sites. The site for `pkgdown` itself is a good place to start, naturally.

The big thing you'll want to do is organize the "reference" (the help pages), grouping by topic and ordering them sensibly. [By default](https://github.com/hadley/pkgdown/blob/081639735104a03c01527f568a99f0ef7351433d/R/build-reference-index.R#L86), all exported, non-internal (that's `@keywords internal` in [roxygen](https://github.com/klutometis/roxygen)-speak) functions are included, but you don't have to keep everything in your index. Any function you omit from the index will still exist on your site, so you can cross-reference to it, but it won't appear in the index page.
...
For sensibly named files, the quoting is unnecessary, and you won't see it in [good examples](https://github.com/hadley/pkgdown/blob/081639735104a03c01527f568a99f0ef7351433d/_pkgdown.yml) in the wild.
