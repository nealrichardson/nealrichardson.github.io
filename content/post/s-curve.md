...
Over Labor Day weekend, Caltrans closed the bridge, and the contractor demolished the old section of bridge and then literally slid the new structure into place. For the next four years, drivers passed over this S-Curve, until the new bridge opened. Then the S-Curve was demolished, along with the rest of the old bridge.

# Lessons

I think about the bridge construction a lot, and not just because I spent ten years driving across the old one, terrified that it would collapse under me, hopeful that the new one would finish soon. The project, and the S-Curve in particular, illustrate key points that we had been missing in our development process.
...
The following discussion is focused on changing an API, though you could generalize to any kind of data contract or promise that your system makes that you want to alter. In our case, we have a web application that consumes our API, but we also have R and Python libraries that wrap it, and we even have some users who directly communicate with the API outside of our libraries.
...
Our developers are great: they're smart, they're motivated to build good software that solves real problems, and they're determined to write high-quality code. We don't like to "hack": we prefer to take the extra minute to think about the right way to solve a problem and make sure that we have test coverage for whatever we add. We love deleting dead code and simplifying things so that we can delete code. These are great cultural norms, and we've worked hard to cultivate them.
...
Disposable structures like the S-Curve---or even more extreme, the support structure built on Yerba Buena Island onto which the S-Curve was constructed prior to sliding it into place---have different standards for excellence since they don't need to survive for as long. They can be hackier and uglier. It turned out, though, that while we like deleting code, we don't like writing code that we plan to delete. It went against our values.

It's important to know when you need a robust solution and when you need a hack. One could say that that wisdom is a quality that distinguishes a truly senior engineer. Nevertheless, the challenges we faced and the lessons we learned make it clear that our problem was not a lack of developer maturity. Where we lacked a clear project plan, we were on the wrong track from the beginning. Our team faithfully pursued the product requirements they were given---we had just failed to give them the final requirement of "Do all that and don't break shit for everyone while you're doing it."
